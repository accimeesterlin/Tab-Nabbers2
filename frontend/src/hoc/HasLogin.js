...
const mainJoin = (methods, name, url) => WrappedComponent => {
  class MainJoin extends React.Component {
    ...
    submit = event => {
      ...
      const { email, password, name } = this.props.user;

      this.props.dispatch(
        methods.joinOrLogin(url, {
          email,
          password,
          name
        })
    ...
    getValues = ({ target: { name, value } }) => {
      const data = { [name]: value };
      this.props.dispatch(methods.getValues(data));
    };

    componentWillReceiveProps(props) {
      if (props.user.authenticated && name === "join") {
        this.props.history.push("/welcome/basic");
